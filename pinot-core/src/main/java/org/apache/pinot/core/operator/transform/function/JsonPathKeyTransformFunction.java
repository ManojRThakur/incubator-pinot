...
/**
 * The <code>JsonPathKeyTransformFunction</code> class implements the json path key transformation based on
 ...
 *
 * Usage:
 * jsonPathKey(jsonFieldName, 'jsonPath')
 ...
 *
 */
public class JsonPathKeyTransformFunction extends BaseTransformFunction {

  public static final String FUNCTION_NAME = "jsonPathKey";
  ...
  public void init(@Nonnull List<TransformFunction> arguments, @Nonnull Map<String, DataSource> dataSourceMap) {
    ...
    if (arguments.size() != 2) {
      throw new IllegalArgumentException(
          "Exactly 2 arguments are required for transform function: jsonPathKey(jsonFieldName, 'jsonPath')");
    }

    TransformFunction firstArgument = arguments.get(0);
    if (firstArgument instanceof LiteralTransformFunction || !firstArgument.getResultMetadata().isSingleValue()) {
      throw new IllegalArgumentException(
          "The first argument of jsonPathKey transform function must be a single-valued column or a transform function");
    }
