  ...
  public void init(@Nonnull List<TransformFunction> arguments, @Nonnull Map<String, DataSource> dataSourceMap) {
    ...
    if (arguments.size() < 3 || arguments.size() > 4) {
      throw new IllegalArgumentException(
          "Expected 3/4 arguments for transform function: jsonPath(jsonFieldName, 'jsonPath', 'resultsType', ['defaultValue'])");
    }

    TransformFunction firstArgument = arguments.get(0);
    if (firstArgument instanceof LiteralTransformFunction || !firstArgument.getResultMetadata().isSingleValue()) {
      throw new IllegalArgumentException(
          "The first argument of jsonPath transform function must be a single-valued column or a transform function");
    }
    ...
    try {
        ...
        switch (fieldType) {
          ...
          case BYTES:
            throw new UnsupportedOperationException(String.format(
                "Unsupported results type: BYTES for 'jsonPath' Udf. Supported types are: INT/LONG/FLOAT/DOUBLE/STRING/INT_ARRAY/LONG/FLOAT_ARRAY/DOUBLE_ARRAY/STRING_ARRAY",
                _resultsType));
        }
      }
      ...
      throw new UnsupportedOperationException(String.format(
          "Unsupported results type: %s for 'jsonPath' Udf. Supported types are: INT/LONG/FLOAT/DOUBLE/STRING/INT_ARRAY/LONG/FLOAT_ARRAY/DOUBLE_ARRAY/STRING_ARRAY",
          _resultsType));
    }
  }
